...
export async function postRun(req, res) {
    ...
    // 3) Call AI to get optimized output (stubbed function returns structured result)
    let optimized;
    try {
...
/**
 * GET /api/v1/runs
 * Return recent completed runs (joined with optimized_data and original_data).
...
export async function getRuns(req, res) {
    try {
        ...
        // Select runs with joined optimized_data and original_data (if available)
        const rows = await db
            .select({
            run_id: runs.id,
            asin: runs.asin,
            ...
            .limit(limit)
            .offset(offset);
        // Filter only completed runs (those with optimized_id present)
        ...
        const result = completed.map((r) => ({
            ...
            optimized: r.optimized_id
                ? {
                    ...
                    bullets: r.optimized_bullets ?? [],
                    description: r.optimized_description ?? null,
                    keywords: r.optimized_keywords ?? [],
