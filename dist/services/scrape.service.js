...
function parseAplusDescription($) {
    const selectors = [
...
function cleanText(str) {
    ...
    if (/p\.when\(|a\.on\(|window\.ue\b|window\.csa\b|csa\(|execute\(function|\bfunction\s*\(|var\s+\w+\s*=|<\/?script\b/i.test(lower)) {
        return null;
    }
    ...
    const numericRatio = (text.match(/\d+/g) || []).join("").length / Math.max(1, text.replace(/\s+/g, "").length);
    ...
    if (finalSpecHits >= 5 && normalized.length > 100)
        return null;
    return normalized || null;
}
function cleanBullets(raw, opts) {
    if (!raw)
        return [];
    ...
    for (let part of rawParts) {
        ...
        if (/\bout of 5 stars\b|\bverified purchase\b|\brated\b/i.test(lower))
            continue;
        if (/(function\(|window\.ue\b|P\.when\(|dpAcrHasRegisteredArcLinkClickAction|ue\.count\()/i.test(part))
            continue;
        if (/amazon-avatars|m\.media-amazon|img src=/i.test(part))
            continue;
        ...
        if (/^\s*(best sellers rank|customer reviews|customer rating|ratings|best sellers)\b/i.test(part))
            continue;
        let candidate = part.replace(/\s+/g, " ").trim();
        if (candidate.length < 8 && !/[A-Za-z]/.test(candidate))
            continue;
        if (/\b(reviewed in|read more|helpful report|verified purchase)\b/i.test(candidate))
            continue;
        ...
        for (const part of rawParts) {
            ...
            if (p.length >= 12 && p.length <= 180 && /[A-Za-z]/.test(p) && !/review|rating|best sellers|asin|item model/i.test(p)) {
                if (!cleaned.includes(p))
                    cleaned.push(p);
                if (cleaned.length >= Math.min(4, maxBullets))
                    break;
            }
        }
    }
    const normalizedBullets = cleaned.map((b) => {
        const t = b.trim();
        if (/[a-zA-Z]$/.test(t) && t.length > 40 && !/[.?!]$/.test(t))
            return t + ".";
        return t;
    });
...
export async function scrapeAmazonProduct(asin) {
    ...
    try {
        ...
        // title
        let title = norm($("#productTitle").text()) ||
            norm($("#title").text()) ||
            norm($("h1#title").text()) ||
        ...
        let bullets = cleanBullets(rawBullets);
        if (bullets.length === 0) {
        ...
        // description
        let description = norm($("#productDescription").text()) ||
