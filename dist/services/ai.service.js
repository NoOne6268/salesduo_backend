...
Do NOT include any explanation or extraneous text. If a field cannot be produced, return an empty string or empty array. Avoid unsubstantiated claims.`;
    ...
    if (input.bullets && input.bullets.length)
        parts.push(`Original bullets:\n- ${input.bullets.join("\n- ")}`);
...
function extractTextFromResponse(resp) {
    try {
        ...
        if (Array.isArray(out) && out.length > 0) {
            ...
            if (typeof out[0]?.text === "string")
                return out[0].text;
            if (typeof out[0]?.content?.[0]?.text === "string")
                return out[0].content[0].text;
        }
        if (typeof resp?.output_text === "string")
            return resp.output_text;
        if (typeof resp?.generated_text === "string")
...
export async function optimizeProduct(input) {
    ...
    try {
        ...
        const keywords = Array.isArray(parsed.keywords) ? parsed.keywords.map((k) => String(k).trim()) : [];
        return {
            title,
            bullets,
            description,
